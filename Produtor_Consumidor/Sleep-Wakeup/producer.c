...
void insert_item(int *count)
{	
	*count = *count + 1;
	printf("Item inserido\n");
}
void produce_item(int *count, int *a)
{
	if (*count < N)
	{
		insert_item(count);	
...
int main ()
{
	...
	*proc_var_A = 1; //iniciando proc_var_A com 1
	...
	/* Acoplar proc_var_b count a um segmento de memória compartilhado. */
	...
	*proc_var_B = 0; //iniciando proc_var_B com 0
	...
	{	
		if (*proc_var_A == 1)
		{
			produce_item(count, proc_var_A);
		}
		else
		{
			sleep(2);//sleep(producer)
		}
		
		if (*count == 1)
		{
			
			*proc_var_B = 1; //wakeup(consumer)
		}
